...
public abstract partial class SharedStationAiSystem : EntitySystem
{
    ...
    private void OnIntellicardDoAfter(Entity<StationAiHolderComponent> ent, ref IntellicardDoAfterEvent args)
    {
        ...
        if (_slots.CanEject(ent.Owner, args.User, ent.Comp.Slot))
        {
            ...
            if (ent.Comp.Slot.Item != null
                ...
                && stationAiHeldComp.CurrentConnectedEntity is not null)
            {
                var entityUid = (EntityUid)stationAiHeldComp.CurrentConnectedEntity;
                _remoteSystem.ReturnMindIntoAi(entityUid);
            }
    ...
    private void OnHolderInteract(Entity<StationAiHolderComponent> ent, ref AfterInteractEvent args)
    {
        ...
        if (TryComp<StationAiHeldComponent>(held, out var heldComp))
            if (heldComp.CurrentConnectedEntity is EntityUid entityUid)
                AnnounceIntellicardUsage(entityUid, intelliComp.WarningSound);
        // Corvax-Next-AiRemoteControl-End
